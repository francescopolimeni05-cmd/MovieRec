...
ax.bar(dist.index, dist.values, width=0.7, color="#4C72B0")
...
axes[0].hist(np.clip(upc, None, clip), bins=60, color="#55A868")
...
ax.barh(genres.index[::-1], genres.values[::-1], color="#8172B3")
...
m1.plot(kind="bar", ax=axes[0], color=["#4C72B0", "#55A868", "#C44E52"], legend=False)
...
ax_nov = axes[1].twinx()
m2cov.plot(kind="bar", ax=axes[1], color=["#8172B3"], width=0.4, position=1, legend=False)
metrics.loc[order, "Novelty"].plot(kind="bar", ax=ax_nov, color="#CCB974",
...
axes[1].legend(handles=[Patch(color="#8172B3", label="Coverage"),
                        Patch(color="#CCB974", label="Novelty")],
